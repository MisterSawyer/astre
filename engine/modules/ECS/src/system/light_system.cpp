...
namespace astre::ecs::system
{
    ...
    asio::awaitable<void> LightSystem::run(render::Frame & frame)
    {
        ...
        { // collect lights
            ...
            {
                if(light_id >= MAX_LIGHTS) break;

                if (!mask[MASK_BIT] || !mask[TransformSystem::MASK_BIT]) continue;

                light_component = registry.getComponent<LightComponent>(entity);
                transform_component = registry.getComponent<TransformComponent>(entity);
                assert(light_component != nullptr && transform_component != nullptr);
